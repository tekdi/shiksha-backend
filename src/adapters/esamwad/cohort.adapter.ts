...
import { StudentCohortMembershipDto } from "src/cohort/dto/studentCohortMembership.dto";
import { UserDto } from "src/user/dto/user.dto";
import { CohortSearchDto } from "src/cohort/dto/cohort-search.dto";
import { StudentDto } from "src/student/dto/student.dto";
export const EsamwadCohortToken = "EsamwadCohort";

@Injectable()
export class EsamwadCohortService implements IServicelocatorcohort {
  ...
  public async findMembersOfCohort(id: string, role: string, request: any) {
    if (role == "Student") {
      var axios = require("axios");
      var data = {
        query: `query getcohortStudents($grade_number: Int!,$limit:Int!) {
  ...
  public async studentMappedResponse(result: any) {
    const studentResponse = result.map((obj: any) => {
      const studentMapping = {
        ...
        cohortId: obj?.grade_number ? `${obj.grade_number}` : "",
        ...
        bloodCohort: obj?.bloodCohort ? `${obj.bloodCohort}` : "",
