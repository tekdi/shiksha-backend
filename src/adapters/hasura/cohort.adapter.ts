...
export class HasuraCohortService implements IServicelocatorcohort {
  ...
  public async createCohort(request: any, cohortCreateDto: CohortCreateDto) {
    try {
        ...
        // Constructing the input object for the mutation
        let inputObject = {};
        Object.keys(cohortCreateDto).forEach((key) => {
            if (cohortCreateDto[key] !== undefined && key !== 'fieldValues') {
                inputObject[key] = cohortCreateDto[key];
            }
        });

        // Include params field if it exists
        if (cohortCreateDto.params) {
            inputObject['params'] = cohortCreateDto.params;
        }

        // Constructing the mutation query
        var data = {
            query: `mutation CreateCohort($input: Cohort_insert_input!) {
                insert_Cohort_one(object: $input) {
                    cohortId
                }
            }`,
            variables: {
