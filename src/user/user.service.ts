...
export class UserService {
    ...
    try {
      ...
      response
        ...
        .send(
          APIResponse.error(
            apiId,
            'Something went wrong In finding UserDetails', e,
  ...
  //   // Add contextType condition if it's provided in userData
  //   if (userData.contextType) {
  //     whereCondition.contextType = userData.contextType.toUpperCase();
  //   }
  ...
  async findFilledValues(userId: string) {
    let query = `SELECT U."userId",F."fieldId",F."value",U."role" FROM public."Users" U 
